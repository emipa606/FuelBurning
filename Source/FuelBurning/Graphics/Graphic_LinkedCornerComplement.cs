...
namespace FuelBurning
{
    internal class Graphic_LinkedCornerComplement : Graphic_Linked
    {
        ...
        {
            if (i == 0 && (!ShouldLinkWith(position + IntVec3.West, thing) ||
                           !ShouldLinkWith(position + IntVec3.South, thing)))
            {
                return false;
            }

            if (i == 1 && (!ShouldLinkWith(position + IntVec3.West, thing) ||
                           !ShouldLinkWith(position + IntVec3.North, thing)))
            {
                return false;
            }

            if (i == 2 && (!ShouldLinkWith(position + IntVec3.East, thing) ||
                           !ShouldLinkWith(position + IntVec3.North, thing)))
            {
                return false;
            }

            if (i != 3 || ShouldLinkWith(position + IntVec3.East, thing) &&
                ShouldLinkWith(position + IntVec3.South, thing))
            {
                return true;
            }
        ...
        protected Material LoadCompMaterial(string localPath)
        {
            ...
            Log.Error(DebugLog.Sign() + "Could not found the texture. path \"" + compTexPath + "\"");
